...
class App extends Object {
	...
	*/
	...
	public static function import($type = null, $name = null, $parent = true, $search = array(), $file = null, $return = false) {
		...
		//echo $name . $ext['class']."gfjfjf";
		if ($name != null && !class_exists($name . $ext['class'])) {

			//var_dump($_this->__mapped($name . $ext['class'], $type, $plugin));
	...
	function __load($file) {
		...
		//echo $file2;
		//$file=preg_replace('/(?<=\/)([a-z])(?!.*\/)/e',$replace, $file);
		//echo $file;
		//$file=preg_replace('/(?<=_)([a-z])(?!.*\/)/e',$replace, $file);
		//echo $file;
		if (file_exists($file)) {
			if (!$this->return) {
				require($file);
		...
		$file=preg_replace('/(?<=_)([a-z])(?!.*\/)/e',$replace, $file);
		$file2=preg_replace('/(?<=\/)([a-z])(?!.*\/)/e',$replace, $file);
		...
		//$file=preg_replace('/(?<=\\w)([A-Z])|^([A-Z])/e', strtoupper('_\\$1'), $file);
