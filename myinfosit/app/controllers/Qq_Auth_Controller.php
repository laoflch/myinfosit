...
class QqAuthController extends AppController implements NoModelController
{
	var $name = 'Authkey';
	//var $name = 'User';
	...
	function UserAuth() {
		...
		//echo var_dump($this);
		$auth_key=& $this->Authkey->findBySource_name("qq");
		...
		$adapter=& AdapterHandle::GetAdapter("QqAdapter.Qqoauthv2Adapter");
		...
		$url=$adapter->DoAuth($auth_key['akey'],$auth_key['skey'],$this->Session,"http://www.yonganfish.com/myinfosit/QqAuth/CallBack");
		//headers_sent($file, $line) ;
		//echo $file.$line;
		$this->redirect($url);
		//echo var_dump($auth_key);
	...
	function CallBack() {
		...
		$url=$adapter->GetAccessToken($this->Session,"http://www.yonganfish.com/myinfosit/QqAuth/CallBack");
		//$this->redirect($url);
