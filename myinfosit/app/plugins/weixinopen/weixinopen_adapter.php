...
file_put_contents("test37.txt",$var,FILE_APPEND); 
	...
	static function fetch_access_token($app_id,$app_secret){
		...
		isset($app_secret)&&!empty($app_secret)){
			...
			curl_setopt($curl, CURLOPT_URL,"https://api.weixin.qq.com/cgi-bin/token?".$data);
			curl_setopt($curl, CURLOPT_SSL_VERIFYPEER, 0); // 对认证证书来源的检查
			curl_setopt($curl, CURLOPT_SSL_VERIFYHOST, 0); // 从证书中检查SSL加密算法是否存在
			curl_setopt($curl, CURLOPT_USERAGENT, $_SERVER['HTTP_USER_AGENT']); // 模拟用户使用的浏览器
			curl_setopt($curl, CURLOPT_FOLLOWLOCATION, 1); // 使用自动跳转
	...
	static function update_self_menu($access_token,$menu_str){
		...
		isset($menu_str)&&!empty($menu_str)){
			...
			curl_setopt($curl, CURLOPT_POST, 1); // 发送一个常规的Post请求
			curl_setopt($curl, CURLOPT_POSTFIELDS, $menu_str); // Post提交的数据包
			curl_setopt($curl, CURLOPT_TIMEOUT, 30); // 设置超时限制防止死循环
			curl_setopt($curl, CURLOPT_HEADER, 0); // 显示返回的Header区域内容
			curl_setopt($curl, CURLOPT_RETURNTRANSFER, 1); // 获取的信息以文件流的形式返回
			//curl_setopt($curl, CURLOPT_REFERER, "https://mp.weixin.qq.com/");
